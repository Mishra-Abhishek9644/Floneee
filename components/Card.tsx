...
const Card = ({ product, onOpen }: cardProp) => {
  ...
  // ⏳ existing debounce ref (kept as requested)
    ...
    <div className="p-4 flex flex-col mt-3 justify-center group relative z-0">
      ...
      <div className="absolute inset-x-0 bottom-18 h-12 mx-4 hidden items-center bg-purple-500 opacity-0 group-hover:opacity-100 translate-y-3 group-hover:translate-y-0 transition-all duration-700 ease-out md:flex z-10">
        <button
          ...
          className={`w-12 h-full flex items-center justify-center text-white hover:bg-black ${isInWishlist ? "bg-black" : ""
            }`}
        >
        ...
        <button
          onClick={onOpen}
          className="w-12 h-full flex items-center justify-center text-white hover:bg-black"
        >
          <Eye size={18} />
        </button>
      </div>
